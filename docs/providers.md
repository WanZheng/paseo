...
OMP approval modes are surfaced as Paseo agent modes: `always-ask`, `write`, and `yolo`. Paseo passes the initial mode through OMP's `--approval-mode` launch flag. When supported by OMP, runtime mode changes use the RPC `set_approval_mode` command to update `tools.approvalMode` in the running process; older OMP builds fall back to restarting the RPC process with the same native session file and the new `--approval-mode` value.

Pi RPC extension UI dialog requests (`select`, `input`, `editor`, `confirm`) are bridged into Paseo question permissions and answered with `extension_ui_response`. Pi-family extensions such as `ask_user` may chain dialogs: for example, a `select` can be followed by an optional-comment `input`. When an `ask_user` tool call declares `allowComment: true`, Paseo presents the selection and optional comment as one question permission, answers the initial `select` immediately, then auto-answers the follow-up optional `input` with the comment the user already supplied (or an empty string). Preserve placeholders and optional/skip semantics for standalone optional inputs so the app can still distinguish "skip this optional input" from "cancel the whole dialog." Fire-and-forget extension UI requests such as notifications are intentionally ignored by the provider adapter unless Paseo grows first-class UI for them.
